...
const ProductPrice = ({ value, currency = "$", className }: { value: number; currency?: string; className?: string }) => {
    const sign = value < 0 ? "-" : "";
    const [integerPart, decimalPart] = Math.abs(value).toFixed(2).split(".");

    return (
        <p className={cn("inline-flex items-baseline text-2xl font-bold tracking-tight tabular-nums", className)}>
